...
int main(int argc, char **argv) {
	...
	// fill matrix
	srand(7);
    #pragma omp parallel for schedule(auto) collapse(1) shared(a,b)
	for (long i = 0; i < n; ++i) {
		for (long j = 0; j < n; ++j) {
			a[i][j] = rand();
			b[i][j] = rand();
...
#pragma omp parallel for schedule(auto) collapse(2) shared(a,b,c)
		for (long i = 0; i < n; ++i) {
			for (long j = 0; j < n; ++j) {
				for (long k = 0; k < n; ++k) {
					c[i][j] += a[i][k] * b[k][j];
		...
		// sum of matrix c
   #pragma omp parallel for schedule(auto) collapse(1) shared(local_res)
		for (long i = 0; i < n; ++i) {
			for (long j = 0; j < n; ++j) {
